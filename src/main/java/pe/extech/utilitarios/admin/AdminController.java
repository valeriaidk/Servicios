...
public class AdminController {
        ...
        @Operation(summary = "Listar todos los planes activos", description = "Requiere JWT de un email en `extech.admin.emails`.")
        @ApiResponses({
        ...
        @Operation(summary = "Activar usuario por ID", description = "Llama a `uspUsuarioActivarDesactivar(@Activo=1)`. Registra el ID del admin como `UsuarioModificacion`.")
        @ApiResponses({
        ...
        @Operation(summary = "Desactivar usuario por ID", description = "Llama a `uspUsuarioActivarDesactivar(@Activo=0)`. El usuario no podrá consumir servicios mientras esté inactivo.")
        @ApiResponses({
        ...
        // ─────────────────────────────────────────────────────────────────────────
        @Operation(summary = "[QAS] Cifrar token con AES-256 — ELIMINAR en producción", description = """
        ...
        // ─────────────────────────────────────────────────────────────────────────
        @Operation(summary = "[QAS] Diagnosticar configuración de proveedores externos — ELIMINAR en producción", description = """
        ...
        public ResponseEntity<Object> diagnosticarProveedores() {
                ...
                List<Map<String, Object>> diagnostico = rows.stream().map(row -> {
                        ...
                        Integer tokenLen = row.get("TokenLongitud") != null
                                        ? ((Number) row.get("TokenLongitud")).intValue()
                                        : 0;
                        ...
                        if (endpoint != null && endpoint.contains("?")) {
                                ...
                                urlSimulada = endpoint + "?" + paramNombre + "=" + valorEjemplo
                                                + "  [parámetro estándar]";
                        }
                        ...
                        item.put("autorizacionTienePlaceholder",
                                        autorizacion != null && autorizacion.contains("{TOKEN}"));
        ...
        // ─────────────────────────────────────────────────────────────────────────
        @Operation(summary = "Crear o actualizar configuración de API externa (cifra el token automáticamente)", description = """
        ...
        public ResponseEntity<Object> guardarApiExterna(
                ...
                try {
                        ...
                        MapSqlParameterSource params = new MapSqlParameterSource()
                                        ...
                                        .addValue("TiempoConsulta",
                                                        req.tiempoConsulta() != null ? req.tiempoConsulta() : 60)
                                        .addValue("SegmentoTiempo",
                                                        req.segmentoTiempo() != null ? req.segmentoTiempo() : "SEG")
        ...
        public record ApiExternaGuardarRequest(
                        ...
                        @NotBlank(message = "codigo es obligatorio (ej: DECOLECTA_RENIEC)") String codigo,

                        String descripcion,

                        @NotBlank(message = "endpoint es obligatorio") String endpoint,

                        @Pattern(regexp = "GET|POST|PUT|PATCH|DELETE", flags = Pattern.Flag.CASE_INSENSITIVE, message = "metodo debe ser GET, POST, PUT, PATCH o DELETE") String metodo,

                        @NotBlank(message = "token es obligatorio (texto plano — el backend lo cifra)") String token,
        ...
        // ─────────────────────────────────────────────────────────────────────────
        @Operation(summary = "Actualizar configuración de API externa por ID", description = """
        ...
        public ResponseEntity<Object> actualizarApiExterna(
                        ...
                        try {
                                ...
                                if (rows.isEmpty()) {
                                        return ResponseEntity.status(404)
                                                        .body(Map.of("ok", false, "mensaje",
                                                                        "No se encontró el proveedor con código: "
                                                                                        + req.codigo()));
                                }
                ...
                try {
                        ...
                        if (tokenActualizado)
                                respuesta.put("tokenCifradoLen", tokenParaSP.length());
        ...
        @Operation(summary = "Estadísticas globales de consumo del mes actual", description = "Cuenta todos los consumos exitosos del mes en curso, sin filtrar por usuario.")
        @ApiResponses({

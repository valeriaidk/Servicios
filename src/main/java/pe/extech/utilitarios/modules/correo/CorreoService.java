 ...
 * Extiende EnvioBaseService: validación de plan, templates y consumo = idéntico
 * a SmsService (R4).
 *
 * Fuentes de configuración:
 * - clientId, tenantId, outlookUser → application.properties (@Value)
 * - clientSecret → IT_ApiExternaFuncion.Token (cifrado AES-256,
 * Codigo='MICROSOFT_GRAPH_CORREO')
 *
 ...
 * 3. POST graph.microsoft.com/v1.0/users/{outlookUser}/sendMail con Bearer
 * access_token
 *
 * Seguridad (R6/R8):
 * - access_token NO se persiste ni se loguea. Vive solo en memoria durante el
 * request.
...
public class CorreoService extends EnvioBaseService implements ICorreoService {
    ...
    private static final String GRAPH_TOKEN_TEMPLATE = "https://login.microsoftonline.com/%s/oauth2/v2.0/token";
    private static final String GRAPH_SEND_MAIL_TEMPLATE = "https://graph.microsoft.com/v1.0/users/%s/sendMail";
     ...
     * El valor NUNCA se loguea ni persiste — se usa directamente como Bearer
     * header.
     */
     ...
     *
     * @param accessToken Token OAuth2 obtenido en runtime — no persistido ni
     *                    logueado.

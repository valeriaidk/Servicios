...
public class UsuarioController {
    ...
    @Operation(summary = "Obtener perfil del usuario autenticado", description = "Requiere JWT en el header `Authorization: Bearer {token}`.")
    @ApiResponses({
    ...
    @Operation(summary = "Regenerar API Key manualmente", description = "Llama a `uspApiKeyDesactivarYCrear`. El API Key anterior queda "
            +
    ...
    @Operation(summary = "Cambiar plan de suscripción", description = "Llama a `uspPlanUsuarioCambiar` en una transacción. Retorna el nuevo JWT "
            +
    ...
    @Operation(summary = "Historial paginado de consumos", description = "Parámetros opcionales: `page` (default 1) y `size` (default 20).")
    @ApiResponses({
    ...
    @Operation(summary = "Resumen del consumo mensual actual vs límite del plan", description = "Retorna consumo actual, límite máximo y porcentaje usado por función.")
    @ApiResponses({

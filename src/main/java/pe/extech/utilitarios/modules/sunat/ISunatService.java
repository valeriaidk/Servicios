 ...
 *
 * Implementación actual: {@link SunatService} — consulta el RUC mediante el
 * proveedor
 * Decolecta. Retorna razón social, estado tributario, condición (habido/no
 * habido),
 ...
 * <li>Validación local del RUC (11 dígitos, prefijo 10 o 20) — sin consumir el
 * plan.</li>
 * <li>{@code uspResolverApiExternaPorUsuarioYFuncion(@UsuarioId, 'SUNAT_RUC')}
 * — resuelve
 * endpoint, token AES-256 y {@code ApiServicesFuncionId} desde
 * {@code IT_ApiAsignacion}.</li>
 * <li>{@code uspPlanValidarLimiteUsuario} — corta con 429 si el límite mensual
 * está agotado.</li>
 * <li>HTTP GET a Decolecta con el token descifrado. Timeout configurable
 * (default 60 s).</li>
 ...
 * </ol>
 *
 * Extensibilidad: si en el futuro se integra un segundo proveedor, basta con
 * crear
 * una nueva clase que implemente esta interfaz y anotarla con {@code @Primary}.
 * {@link SunatController} y cualquier componente que dependa de
 * {@code ISunatService}
...
public interface ISunatService {
     ...
     *
     * <p>
     * El RUC debe tener exactamente 11 dígitos y comenzar con {@code 10} (persona
     * natural)
     * o {@code 20} (persona jurídica). Si el RUC tiene formato válido pero SUNAT no
     * tiene
     * datos para él, Decolecta retorna campos vacíos; el consumo se registra
     * igualmente (R2).
     * </p>
     *
     ...
     * @param numeroRuc Número de RUC de 11 dígitos. La validación de formato se
     *                  realiza
     *                  antes de llamar al proveedor para evitar cobros por datos
     *                  inválidos.

 ...
 * Extiende EnvioBaseService para reutilizar lógica de plan, template y consumo
 * (R4).
 *
 * Fuente de configuración del proveedor:
 * - endpoint, token (AES-256), autorizacion → IT_ApiExternaFuncion via
 * uspResolverApiExternaPorUsuarioYFuncion
 * - sender-id por defecto → application.properties
 * (extech.proveedor.infobip.sender-id)
 *
...
public class SmsService extends EnvioBaseService implements ISmsService {
    ...
    public SmsResponse enviar(int usuarioId, SmsRequest request) {
        ...
        // descifrarConFallback() maneja tanto tokens cifrados AES-256 como texto plano
        // legacy.
     ...
     * (ej: "App {TOKEN}") con el placeholder {TOKEN} reemplazado por el token
     * descifrado.
     ...
     * "messages": [{
     * "messageId": "...", "to": "+51...",
     * "status": { "id": 26, "name": "PENDING_ACCEPTED", "groupId": 1, "groupName":
     * "PENDING",
    ...
    /**
     * Parsea la respuesta real de Infobip y mapea los campos al SmsData de la
     * respuesta.
     *
     * Si la respuesta es null o no contiene "messages", retorna SmsData con campos
     * null

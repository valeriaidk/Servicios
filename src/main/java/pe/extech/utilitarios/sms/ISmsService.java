 ...
 *
 * Implementación actual: {@link SmsService} — envía mensajes de texto vía Infobip.
 * Soporta dos modos de contenido:
 * <ul>
 *   <li><b>INLINE</b>: el campo {@code message} del request se usa directamente.</li>
 *   <li><b>TEMPLATE</b>: se resuelve la plantilla desde {@code IT_Template} (Canal=SMS)
 *       mediante {@code uspTemplateObtenerAsunto} y se sustituyen las variables
 *       {@code {{clave}}} con el mapa {@code variables} del request.</li>
 * </ul>
 *
 * Lógica compartida con {@link pe.extech.utilitarios.correo.ICorreoService}:
 ...
 *   <li>{@code uspResolverApiExternaPorUsuarioYFuncion(@UsuarioId, 'SMS_SEND')} — resuelve
 *       endpoint Infobip, token AES-256, template de autorización ({@code App {TOKEN}})
 ...
 *   <li>Validación local del teléfono (formato {@code +51XXXXXXXXX}) — sin consumir el plan.</li>
 *   <li>Resolución de contenido según modo (TEMPLATE o INLINE).</li>
 *   <li>HTTP POST a Infobip con {@code Authorization: App <token_descifrado>}. Timeout: 30 s.</li>
 *   <li>{@code uspConsumoRegistrar} — registra en {@code IT_Consumo} con {@code EsConsulta=0},
 *       siempre, tanto en éxito como en fallo (R2).</li>
 * </ol>
 *
 * Extensibilidad: para agregar un segundo proveedor (ej: Twilio, AWS SNS) basta con
...
public interface ISmsService {
     ...
     *
     * <p>El campo {@code mode} del request determina el origen del contenido:
     * {@code INLINE} usa {@code message} directamente; {@code TEMPLATE} resuelve
     * el cuerpo desde {@code IT_Template} y sustituye las variables {@code {{clave}}}.</p>
     *
     * <p>La respuesta incluye los datos reales de Infobip: {@code messageId},
     * {@code statusId}, {@code statusName}, {@code statusGroupName} y
     * {@code statusDescription}.</p>
     *

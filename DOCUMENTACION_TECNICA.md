...
- Implementación dentro de segurity `JwtUtil` (firma HS256, claims mínimos: `userId` y `planId`).
...
│       │   │   │
│       │   │   ├── reniec/
│       │   │   │   ├── IReniecService.java          ← Contrato del servicio: consultarDni()
...
│       │   │   │
│       │   │   ├── sunat/
│       │   │   │   ├── ISunatService.java           ← Contrato del servicio: consultarRuc()
...
│       │   │   │
│       │   │   ├── sms/
│       │   │   │   ├── ISmsService.java             ← Contrato del servicio: enviar()
...
│       │   │   │
│       │   │   ├── correo/
│       │   │   │   ├── ICorreoService.java          ← Contrato del servicio: enviar()
...
#### `reniec/IReniecService.java`
 
```
pe.extech.utilitarios.reniec.IReniecService
```

**Qué es:** Interfaz Java que define el **contrato** del servicio de consulta RENIEC. Declara un único método: `consultarDni(int usuarioId, String numeroDni): ReniecResponse`.

**Para qué sirve:** Desacopla el Controller de la implementación concreta. `ReniecController` inyecta `IReniecService` — no `ReniecService` directamente. Si en el futuro se cambia el proveedor de consulta DNI (de Decolecta a otro), solo hay que crear una nueva clase que implemente esta interfaz y marcarla con `@Primary`. El Controller y cualquier otro componente que dependa de `IReniecService` no requiere ningún cambio.

**Implementación actual:** `ReniecService.java` (única implementación activa, marcada con `@Service`). Spring la inyecta automáticamente porque es la única que existe.

**Se relaciona con:** `ReniecController` (la inyecta y usa), `ReniecService` (la implementa).
...
**Se relaciona con:** `IReniecService` / `ReniecService`, `JwtFilter` (establece `userId` en el request), `ApiKeyFilter` (valida la API Key antes de llegar aquí).
...
#### `sunat/ISunatService.java`
 
```
pe.extech.utilitarios.sunat.ISunatService
```

**Qué es:** Interfaz Java que define el **contrato** del servicio de consulta SUNAT. Declara un único método: `consultarRuc(int usuarioId, String numeroRuc): SunatResponse`.

**Para qué sirve:** Igual que `IReniecService` — desacopla el Controller de la implementación. `SunatController` inyecta `ISunatService`, no `SunatService` directamente. Si se integra un segundo proveedor de consulta RUC, basta con implementar esta interfaz sin tocar el Controller.

**Implementación actual:** `SunatService.java` (única implementación activa, marcada con `@Service`).

**Se relaciona con:** `SunatController` (la inyecta y usa), `SunatService` (la implementa).
...
#### `sms/ISmsService.java`
 
```
pe.extech.utilitarios.sms.ISmsService
```

**Qué es:** Interfaz Java que define el **contrato** del servicio de envío de SMS. Declara un único método: `enviar(int usuarioId, SmsRequest request): SmsResponse`.

**Para qué sirve:** Desacopla el Controller del proveedor SMS concreto. `SmsController` inyecta `ISmsService` — si en el futuro se cambia Infobip por Twilio, AWS SNS u otro proveedor, solo se crea una nueva implementación de esta interfaz. El Controller no cambia. El patrón también facilita pruebas unitarias: se puede crear un mock de `ISmsService` sin levantar la integración real.

**Implementación actual:** `SmsService.java` (proveedor Infobip, marcada con `@Service`).

**Se relaciona con:** `SmsController` (la inyecta y usa), `SmsService` (la implementa).
...
**Qué hace:** Expone `POST /api/v1/servicios/sms/enviar`. Recibe `SmsRequest` validado con `@Valid`. Delega a `ISmsService.enviar()`.

**Se relaciona con:** `ISmsService` / `SmsService`, `SmsRequest` (DTO entrada), `SmsResponse` (DTO salida).
...
#### `correo/ICorreoService.java`
 
```
pe.extech.utilitarios.correo.ICorreoService
```

**Qué es:** Interfaz Java que define el **contrato** del servicio de envío de correo electrónico. Declara un único método: `enviar(int usuarioId, CorreoRequest request): CorreoResponse`.

**Para qué sirve:** Igual que `ISmsService` — desacopla el Controller del proveedor de correo concreto. `CorreoController` inyecta `ICorreoService`. Si se quiere cambiar Microsoft Graph por SendGrid directo, AWS SES u otro proveedor, solo se crea una nueva implementación de esta interfaz sin tocar el Controller ni ningún otro componente que la use.

**Implementación actual:** `CorreoService.java` (proveedor Microsoft Graph OAuth2, marcada con `@Service`).

**Se relaciona con:** `CorreoController` (la inyecta y usa), `CorreoService` (la implementa).
